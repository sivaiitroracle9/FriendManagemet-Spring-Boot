...
public class FriendDaoImpl implements FriendDao {
	...
	@Override
	public void addConnection(User u1, User u2) throws DataAccessException {
		...
		Relationship relation = jdbcTemplate.query(
				...
				new Object[] { user1.getId(), user2.getId() }, new int[] {
						Types.VARCHAR, Types.VARCHAR }, new RelationshipRsetExtractor());
		if (relation == null) {
			jdbcTemplate.update("INSERT INTO RELATIONSHIP (ID, FRIEND_ID) VALUES (?, ?)", 
					new Object[]{user1.getId(), user2.getId()}, 
					new int[]{Types.VARCHAR, Types.VARCHAR});
		} else {
			jdbcTemplate.update("UPDATE RELATIONSHIP SET FRIEND_TYPE = ? WHERE ID =? AND FRIEND_ID=?", 
					new Object[]{"FRIEND", user1.getId(), user2.getId()}, 
		...
		if (relation == null) {
			...
			jdbcTemplate.update("UPDATE RELATIONSHIP SET FRIEND_TYPE = ? WHERE ID =? AND FRIEND_ID=?", 
					new Object[]{"FRIEND", user2.getId(), user1.getId()}, 
					new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR});
		}
	...
	@Override
	public void subscribe(User requestor, User target) throws DataAccessException {
	...
	@Override
	public void block(User requestor, User target) throws DataAccessException {
		Relationship relation = jdbcTemplate.query(
				"SELECT * FROM RELATIONSHIP WHERE (ID = ? AND FRIEND_ID = ?)",
				new Object[] {  target.getId(), requestor.getId() }, new int[] {
						Types.VARCHAR, Types.VARCHAR }, new RelationshipRsetExtractor());
		if (relation == null) {
			jdbcTemplate
					.update("INSERT INTO RELATIONSHIP (ID, FRIEND_ID, FRIEND_TYPE, BLOCK) VALUES(?, ?, ?, ?)",
							new Object[] {  target.getId(), requestor.getId(), "SUBSCRIBE", 1 },
							new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
									Types.INTEGER });
		} else {
			jdbcTemplate
	...
	@Override
	public List<String> getConnections(User u) throws DataAccessException {
	...
	@Override
	public List<String> getRecipients(String email) throws DataAccessException {
	...
	private class RelationshipRsetExtractor implements ResultSetExtractor<Relationship> {
		@Override
		public Relationship extractData(ResultSet rs)
				throws SQLException, DataAccessException {
			if(rs.next()) {
				...
				relation.setFriend_type(rs.getString(EntityConstants.friend_type));
				return relation;
			}
			return null;
		}
